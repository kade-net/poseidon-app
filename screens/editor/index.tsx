...
const Editor = (props: Props) => {
  ...
  useEffect(() => {
    if(encodedPost && encodedPost.length > 0){
      try {
        const post: TPUBLICATION = JSON.parse(Buffer.from(encodedPost, 'base64').toString())
        form.reset(post)
      }
      catch (e)
      {
        console.log("Error parsing post", encodedPost);
      }
    }
  }, [encodedPost]);
    ...
    if (!result.canceled) {
      ...
      if (chosen.length > 0) {
        ...
        try {
          ...
          form.setValue("media", [...existingMedia, ...localAssets]);
          ...
          let media = form.getValues("media") ?? [];

          for (const asset of assets) {
            const target = media.findIndex((m) => m.url == asset.localUri);
            if (target > -1) {
              media[target] = asset
            }
          }
            ...
            <YStack w="100%" p={20} pb={0} pt={5} flex={1}>
              <XStack w="100%" columnGap={10}>
                ...
                >
                  ...
                  <YStack flex={1}>
                    <MediaViewer data={CURRENT_MEDIA} onRemove={(id) => {
                      console.log('attempting to remove::', id)
                      const index = parseInt(id)
                      const currentMedia = form.getValues('media')
                      if (currentMedia?.[index]) {
                        const newMedia = currentMedia.filter((m, i) => i !== index)
        ...
        >
          <XStack alignItems={'center'} columnGap={20} >
            <XStack columnGap={5} alignItems='center' >
              <TouchableOpacity
                disabled={CURRENT_MEDIA?.length > 3}
                onPress={handleSelectImage}
              >
                <Image
                  color={
                    CURRENT_MEDIA?.length > 3 ? "$border" : "$button"
                  }
                />
              </TouchableOpacity>
              {
                uploading && <Text color={'$primary'} >
                  Uploading...
                </Text>
              }
            </XStack>

            {publicationType == 1 && <NftCreator/>}
